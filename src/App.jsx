  ...
  return (
    <div ref={ref} className="relative" style={fullWidth ? {} : { width: 180, flexShrink: 0 }}>
      <button
        ...
        className={`w-full flex items-center justify-between text-sm font-medium rounded-lg cursor-pointer focus:outline-none focus:ring-2 focus:ring-green-400 ${dark ?"bg-surface border text-primary" :"text-secondary"}`}
...
function AddPage() {
  ...
  const sel = inp + " sel-arrow";
  const lbl = "block text-sm font-semibold mb-1 text-tertiary";
  const sec = "rounded-xl border p-4 bg-muted";

  const SectionTitle = ({ children }) => (
    <p className="text-sm font-bold uppercase tracking-widest mb-3 text-tertiary">{children}</p>
  );
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl p-5 space-y-4 bg-surface border shadow-xs">
              <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm font-semibold ${form.status ==="Publicado" ? (d ?"bg-green-900/40 text-green-400 -green-700" :"bg-green-50 text-green-700 -green-200") : (d ?"bg-gray-800 text-gray-400 -gray-600" :"bg-gray-100 text-gray-500 border-gray-300")}`}>
...
function EditPage() {
  ...
  const SectionTitle = ({ children }) => (
    <p className="text-sm font-bold uppercase tracking-widest mb-3 text-muted">{children}</p>
  );
      ...
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div className="absolute z-50 mt-1 w-72 rounded-2xl shadow-xl p-4 bg-surface border">
            ...
            <div className="grid grid-cols-7 mb-1">{DAYS.map(day => <div key={day} className="text-center text-sm font-medium py-1 text-muted">{day}</div>)}</div>
            <div className="grid grid-cols-7 gap-y-0.5">
            ...
            <div className="mt-3 pt-3 border-t flex justify-between">
              <button type="button" onClick={() => { const n = new Date(); onChange(`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,"0")}-${String(n.getDate()).padStart(2,"0")}`); setCursor({ y: n.getFullYear(), m: n.getMonth() }); setOpen(false); }} className="text-sm font-medium text-tertiary hover:text-secondary">Hoy</button>
              <button type="button" onClick={() => setOpen(false)} className="text-sm font-medium text-tertiary hover:text-secondary">Cancelar</button>
            </div>
          </div>
        )}
      </div>
              ...
              <div>
                ...
                {form.jira && !jiraLoading && (
                  <div className="mt-2 rounded-lg p-3 bg-muted border">
                    <p className="text-sm font-semibold leading-snug text-primary">{form.jira}</p>
                    {form.jiraStatus && <span className={`inline-block mt-1 text-sm font-bold px-2 py-0.5 rounded ${/done|closed|resolved|finaliz|complet/i.test(form.jiraStatus) ? (d ?"text-green-400 -green-700 bg-green-900/40" :"text-green-700 -green-200 bg-green-50") : (d ?"text-blue-400 -blue-700 bg-blue-900/40" :"text-blue-700 border-blue-200 bg-blue-50")}`}>{form.jiraStatus}</span>}
