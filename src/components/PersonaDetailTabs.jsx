...
export default function PersonaDetailTabs({ personas, type, dark: d, open = true, onToggle }) {
  ...
  return (
    <>
      <div className="rounded-2xl overflow-hidden border shadow-xs bg-surface">
        {/* Banner */}
        <button type="button" onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between gap-3 text-left" style={{ backgroundColor: bannerColor }}>
          ...
          <svg className={`w-5 h-5 text-white/80 flex-shrink-0 transition-transform duration-200 ${open ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>
        </button>

        {/* Grid */}
        {open && <div className={`p-5 grid gap-4 ${personas.length === 1 ? "grid-cols-1 max-w-xs" : personas.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
          {personas.map((p, i) => {
              ...
              <div key={i} className="space-y-2">
                ...
                {img ? (
                  <button onClick={() => setLightbox({ images: allImages, index: globalIdx })}
                    ...
                    <img src={img} alt={`Persona ${i + 1}`} className="w-full h-full object-contain bg-white" />
                  ...
                  <div className="w-full rounded-xl border flex items-center justify-center text-muted text-sm" style={{ aspectRatio: "5/6" }}>
                    Sin imagen
                  </div>
                )}
              </div>
